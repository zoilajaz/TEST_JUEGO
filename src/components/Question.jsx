...
export const Question = ({
	...
	return (
		<>
		<div class="bg-[url('assets/fondo.png')]">
			{activeResults ? (
				<Results
				...
				<div className='flex flex-col justify-between shadow-md  shadow-slate-300 w-[600px] h-[600px] p-10 rounded-lg'>
					...
					<button
						className='border px-5 py-2 rounded-lg font-bold transition-all  bg-cyan-400 hover:bg-yellow-500 hover:text-gray-900'
						onClick={onReset}
					>
						Reiniciar
					</button>
					...
					<div className='grid grid-cols-2 gap-5'>
						...
						{answersRandom.map((answer, index) => (
							<button
								className={`border p-5 rounded-lg flex justify-center items-center bg-cyan-400 hover:scale-105 ${
					...
					{indexQuestion + 1 === questionsFiltered.length ? (
						<button
							className='border-2 border-yellow-600 text-yellow-600 bg-cyan-100 rounded-md px-5 py-2 hover:bg-yellow-600 hover:text-black font-medium'
							onClick={() => {
								setAnswered(false);
